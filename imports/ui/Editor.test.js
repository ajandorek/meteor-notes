...
if (Meteor.isClient) {
  describe('Editor', function () {
    ...
    it('should render note not found', function () {
      const wrapper = mount(<Editor browserHistory={browserHistory} selectedNoteId={notes[0]._id} call={call} />);
      expect(wrapper.find('p').text()).toBe('Note not found.');
    });

    it('should remove note and navigate to dashboard', function () {
      const wrapper = mount(<Editor browserHistory={browserHistory} selectedNoteId={notes[0]._id} call={call} note={notes[0]} />);
